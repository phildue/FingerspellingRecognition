...
def get_likelihood_grid(classifier_score_grid, background_score_grid):
    w_classifier = 0.8
...
def mrf_segmentation(img, img_depth):
    ...
    likelihood_grid = get_likelihood_grid(get_classifier_score(img_segment, pixels_fg, pixels_bg),
    ...
    weight_x = -cv2.normalize(weight_x.astype('float'), None, 0.0, 255.0, cv2.NORM_MINMAX)
    weight_y = -cv2.normalize(weight_y.astype('float'), None, 0.0, 255.0, cv2.NORM_MINMAX)
    likelihood_grid = cv2.normalize(likelihood_grid.astype('float'), None, 0.0, 255.0, cv2.NORM_MINMAX)
